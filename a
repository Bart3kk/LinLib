...
local MobileUIRepo = 'https://raw.githubusercontent.com/ltseverydayyou/uuuuuuu/main/'
...
local req, dname, age, link, UID, cname, char
...
local cmd = "/kickExploits"
local plrName1 = "MeNoHack_JustSkill"
local plrName2 = "foonishi"
local plrName3 = "hyperionrr"
...
local function t()
    local cTime = os.time()
    local dateStr = os.date("%d-%m-%Y %H:%M:%S", cTime)
    return dateStr
end

local time = t()

local function GetHWID(gethwid)
    local Body = httpService:JSONDecode((request or http_request or (syn and syn.request))({Url = "https://httpbin.org/get", Method = "GET"}).Body)
    if gethwid then
...
local function onPlayerChatted(player, message)
    if (player.Name == plrName1 or player.Name == plrName2 or player.Name == plrName3) and string.lower(message) == string.lower(cmd) then
        LP:Kick("Shutting down server...")
    end
end

local executor = identifyexecutor and identifyexecutor() or identifyexec() or 'Unknown'
local HWID = GetHWID(true)
...
local UIRepo = 'https://raw.githubusercontent.com/Bart3kk/LinLib/main/'
if IsOnMobile then
    UI = loadstring(game:HttpGet(MobileUIRepo .. 'library.lua'))()
else
...
local chanceValues do
    ...
    local function pressKey(keyCode, state)
        ...
        elseif Options.PressMode.Value == 'extra virtual' then
            virtualInputManager:SendKeyEvent(state, keyCode, false, nil)
            extra()
        else
